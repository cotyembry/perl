...
open FH, "./HealthPicFY2017_3170426.xml";
open AFH, "./HealthPicFY2017_3170426.xml";
open BFH, "./HealthPicFY2017_3170426.xml";
		...
		if($lineIsCurrent == 1) {
			...
			if($printedOnce == 0) {
				...
				if($inc-1 <= $endOfFileLines[$i]) {
					...
					# ####
					# if($filesToUpdate[$i] eq "MARS.CSP" || $filesToUpdate[$i] eq "MARSLI.csp" || $filesToUpdate[$i] eq "MARSTO.CSP") {
					#	
					# 	#do nothing
					#	
					# }
					...
					# 		print"	S MARSPGTO=0\n";
					# 		print"	I $D(%session.Data(\"MARSDUZ\")) D ACC1^MARS(%session.Data(\"MARSDUZ\"))\n";
					# 		print"    I \'$D(%session.Data(\"MARSDUZ\"))  D\n";
					# 		print"    . I %session.Username\'=\"UnknownUser\" D ACC^MARS(%session.Username)\n";
					# 		print"    . E  S MARSPGTO=1\n";
					# 		print"    I MARSPGTO=1 S %response.Redirect=\"MARSTO.CSP\" Q 1\n";
					# 		print"    Q 1\n";
					# 		print"</script>\n\n\n";
					# 	}						
					# }
					...
					# if($filesToUpdate[$i] eq "MARSPCC.CSP") {
					# 	print "here\n";
					# }

					if($_ =~ m/\<script.*showHideRows\.js/g) {
						if($printedMatchedMessage == 0) {
							$printedMatchedMessage = 1;
							print "Found on the follwing file(s):\n";
						}
						print "$_";

						print "$filesToUpdate[$i] && $startOfFileLines[$i] && $endOfFileLines[$i], $myLineCount\n";
					}
