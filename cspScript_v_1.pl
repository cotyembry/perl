	...
	while(<BFH>) {
		...
		if($lineIsCurrent == 1) {
			...
			if($printedOnce == 0) {
				...
				#if you don't want to do any checking and replacing (i.e. only add something to the page, the just say print"$_"; and move on with your life, now's not the time to do this. That will be later in the code
				...
				if($inc-1 <= $endOfFileLines[$i]) {
					...
					####
						...
						#I may not need this bc of the above check for the html
						#if(($_ =~ m/\<\![ ]*doctype html.*>/gi)  ) {
