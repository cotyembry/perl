			...
			if($printedOnce == 0) {
				...
				if($inc-1 <= $endOfFileLines[$i]) {
					...
					####
						...
						#if the current line has the doctype declaration I want to insert a comment after it
						#I need to account for a <!doctype html> declaration that is on separate lines as well
						if($_ =~ m/\<[ ]*\![ ]*doctype[ ]*html.*\>/gi) { #account for it found on the single line
							#print the comment wanted here
							print"";
							print"";
							print"";
							print"";
